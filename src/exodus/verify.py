...
def expected_report_data(nonce: str) -> bytes:
    return hashlib.sha256(nonce.encode("utf-8")).digest().ljust(_REPORT_DATA_SIZE, b"\0")
...
def verify_url(
    ...
    nonce = fresh_nonce()
    r = httpx.get(f"{base_url.rstrip('/')}/_exodus/attest", params={"nonce": nonce}, timeout=10)
    r.raise_for_status()
    return verify_document(r.json(), nonce, expected_mrenclave, allow_simulated)
